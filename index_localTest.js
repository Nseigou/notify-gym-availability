  ...
  export const handler = async (event) => {
    try {
      ...
      console.error("処理全体でエラーが発生しました:", error); // エラーログを有効化
      return {
        statusCode: 500,
        // エラーオブジェクト全体を返すのではなく、メッセージのみを返すことを推奨
  ...
  async function sendMessage(messageText) {
    if (!token || !groupId) {
      console.error("LINEのアクセストークンまたはユーザーIDが設定されていません。");
      // 環境変数が設定されていない場合はエラーとして処理を中断するか、ログのみに留める
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`LINE APIリクエストエラー: ${response.status} ${response.statusText}`, errorData);
        throw new Error(`LINE APIリクエスト失敗: ${response.status} - ${JSON.stringify(errorData)}`);
      }
      console.log("LINEメッセージ送信成功:", response.status);
    } catch (err) {
      console.error("LINEメッセージ送信中にエラーが発生しました:", err);
      // エラーを再スローするか、適切に処理する
      throw err;
    }
